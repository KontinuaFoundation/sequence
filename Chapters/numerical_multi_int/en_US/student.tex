...
Instead, we use Riemann sums to find an approximate solution.  This is known as \newterm{numerical integration}.
...
\section{Riemann Sums on 2-Dimensional Domains}

When doing Riemann sums on function that takes a single real number, you summed the area of the rectangles under the function to approximate the integral:
...
To do the Riemann sum, we can break the range of the mass into $n_1$ equally sized intervals and the range of the diameter into $n_2$ equally sized intervals.
