...
Consider Ben, who scores the following on ten quizzes:

\[ 8,\ 9,\ 0,\ 10,\ 10,\ 8,\ 7,\ 9,\ 10,\ 5 \]

Ben's total score is
...
Sasha records the daily high temperatures (in degrees Celsius)
...
The mean is the balancing point of the distribution: the point at
which the data would balance if every value were a weight on a
...
Return to Ben's ten quiz scores from the previous section:
...
values, whether unusually high or low, can pull it noticeably away
...
Consider Daniel's car dealership, which tracks its daily sales. Over
14 days, the number of cars sold was:
...
replaced by an exceptional day of 80 cars. The mean would shift upward. The median would not move at all, as the 7th and 8th values in the sorted list are unchanged. This is what it means for the median to be resistant! That is, it does not react to extreme values
...
\begin{enumerate}
    ...
    \item The value 52 represents an unusually high day. It could be a sale
...
The mean and median each have their place, and the choice between them
follows a clear rule. The right choice depends on the shape of the distribution, which you can now read directly from a histogram or boxplot.
...
You saw this in \textit{Introduction to Data Visualization}. Most
...
different stories. What distinguishes them is spread: how far the
...
Because IQR ignores the tails entirely, it is a resistant measure, so outliers do not affect it. When the median is the appropriate measure
...
Let us work through Ben's quiz score data step by step. The ten
scores are $8,\ 9,\ 0,\ 10,\ 10,\ 8,\ 7,\ 9,\ 10,\ 5$ with $\bar{X} = 7.6$.
...
\subsubsection*{Checking Your Work with Python}

Hand computation is worth doing once, so the mechanics stay visible.
After that, software does the arithmetic. Python's built-in
\texttt{statistics} module computes the same sample variance and
standard deviation directly:

% corresponding script: checking_work_with_python.py
\begin{minted}{python}
import statistics
...
This prints \texttt{mean: 7.6}, \texttt{variance: 9.6}, and
\texttt{stdev: 3.0983866769659336}, matching the by-hand result from
Steps 1 through 3. \texttt{statistics.variance} and
\texttt{statistics.stdev} use the sample formulas, dividing by
$n - 1$, which is the same convention this chapter uses. The
population versions, \texttt{statistics.pvariance} and
\texttt{statistics.pstdev}, divide by $n$ instead.

\begin{Exercise}[title={Computing Variance and Standard Deviation},
label={ex:variance_sd}]
Daniel's car dealership records the number of cars sold each day over
14 days:
...
\begin{enumerate} 
    ...
    \bar{x})^2$ for this data set is approximately $743.21$. Use this
...
\begin{enumerate} 
    \item $s^2 = \dfrac{743.21}{14 - 1} = \dfrac{743.21}{13}
    \approx 57.17$ cars$^2$.

    \item $s = \sqrt{57.17} \approx 7.56$ cars.
    \item $s$ is measured in cars. $s^2$ is measured in cars$^2$,
    which has no natural interpretation. This is one reason we prefer the
...
\begin{center}
\begin{tabular}{lll}
Susan: & $8,\ 9,\ 4,\ 8,\ 6,\ 8,\ 7,\ 9,\ 10,\ 5$ &
$s_S \approx 1.90$ \\[2pt]
Joaquin: & $7,\ 8,\ 6,\ 7,\ 6,\ 6,\ 7,\ 5,\ 5,\ 7$ &
$s_J \approx 0.97$ \\
\end{tabular}
\end{center}

Susan's scores vary more from quiz to quiz. Joaquin's are tightly
...
also inflates the standard deviation. Recall Ben's quiz example: the
...
\begin{enumerate} 
    ...
    \item Reine removes all outliers from a right-skewed data
...
Key Points: % Not sure if this should be standard inclusion for all chapters yet, but working on adding this to most chapters and sections -Cheery
\begin{itemize}
...
You have now worked with all four families of numerical measures by
hand: center, spread, shape, and standing. In practice, you will
...
chapter's hand-computed values exactly.
