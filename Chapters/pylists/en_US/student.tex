...
12$ at $x=4$. Now you will write code that does that evaluation.
...
Look over the code and google the functions you aren't familiar
...
pn1 = [-12.0, 10.0, -3.0, 1.0]
y = evaluate_polynomial(pn1, 4.0)
print("y =", polynomial\_to\_string(pn1))
...
\begin{Exercise}[title={Observations}, label=plotobservations]
  ...
  polynomial crosses the x-axis are called \emph{its roots}. Later in
...
\begin{Answer}[ref=plotobservations]
  ...
  would find that it evaluates to zero at each one. Thus, $x=-3$, $x=1$, and $x=2$ are roots.
\end{Answer}
