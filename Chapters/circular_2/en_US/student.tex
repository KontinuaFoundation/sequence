...
We have previously discussed circular motion in the circular motion basics chapter, so let's do a quick review of the important concepts before we move on to rotational dynamics.
...
Alternatively, think of a wrench rotating a large bolt. It will require more force from your arm closer towards the pivot, while pushing at the edge of the wrench will require less force to produce an equivalent torque. Take a look at this in action \href{https://www.youtube.com/watch?v=vQ8etJxpOSw}{here}.

% Door diagram or wrench diagram or both - unplanned at least in TikZ
...
\textbf{Answer}: Before just plugging in values, let's separate the force vector into components. The component that pulls perpendicular to the axis is $50 \sin40^\circ$, while $50 \cos40^\circ$ of the force is applied \textit{pulling} the rod against the pivot, which produces no torque. 
% Diagram made with mathcha
\begin{figure}[H]
\centering
...
The \emph{net torque}\index{net torque} is the sum of all torques present, similar to how net force is the sum of all forces. Generally, counterclockwise torques are positive, while clockwise torques are negative.\footnote{Note that other videos or professors may do this opposite, using CW - CCW, but as long as you are consistent and clear, both are equally valid.} Using our merry go round example, we have $F_2$ is \odot while $F_1$ is \otimes.
\begin{align*}
    \tau_{\text{net}} &= \sum \tau \\
    &= \tau_\text{CCW} - \tau_\text{CW} \\
    &=+(F_2)(r_2) - (F_1)(r_1)
\end{align*}
...
Create an equation for equilibrium of the following forces. Note that you may pick your own pivot point.
...
You can pick your own pivot, so let's choose the point that $\text{A}$ and $\text{B}$ stem from as our pivot. This eliminates those forces, as the $r$ component of the torque would equal zero. Recall that the net torque is $\sum \tau = \tau_\text{CCW} - \tau_\text{CW}$. Analyzing the remaining torques:
\begin{itemize}
  \item $\text{C}$ is $1 \text{ m}$ from the pivot, so the torque is $\tau_\text{C} = (1)\text{C}\sin-90^\circ$. It acts downwards, so our torque is clockwise.
  \item $\text{D}$ acts directly parallel the rotating rod, adding no additional torque on the rod.
  \item $\text{E}$ is $2 \text{ m}$ from the pivot, and acting at a $-135^\circ$ angle from the horizontal. Our torque can be calculated from $\tau_\text{E} = (2)\text{E}\sin-135^\circ$. Note that using $-45^\circ$ provides the same answer. This torque is also clockwise.
\end{itemize}

Our net torque is $\text{C}\sin-90^\circ + (2)\text{E}\sin-135^\circ = 0$
...
\begin{mdframed}[frametitle = {Angular Impulse}, style = important]
  Angular impulse is defined by 
  \begin{equation}
  \Delta \mathbf {L} =\int _{t_{1}}^{t_{2}}{\boldsymbol {\tau }}\,\mathrm {dt}
  \end{equation}
Graphically, this is area under a \emph{torque vs. time} graph. 

Angular impulse has units Newton-meter-seconds, same as angular momentum:
$$\text{N}\cdot \text{m} \cdot \text{s}^{-1} = \text{kg} \cdot \text{m}^2\cdot \text{s}^{-1}$$
\end{mdframed}

Angular impulse explains why sudden twists or torque have effects on rotational objects. A sudden hit to a door, instantaneous wrench forces, or off-center collisions cause a much better impulses. This impulse may be produced either by a large torque applied over a short time or by a smaller torque applied over a longer time. However, in many physical situations, applying a large torque over a near-instantaneous time interval is more effective and practical.
...
\section{Summary}
Here is a summary of linear quantities in physics, and their now introduced rotational components. You may need to refer to this often to so keep it handy.
\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.5}
