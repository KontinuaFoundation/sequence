...
You can pick your own pivot, so let's choose the point that $\text{A}$ and $\text{B}$ stem from as our pivot. This eliminates those forces, as the $r$ component of the torque would equal zero. Recall that the net torque is $\sum \tau = \tau_\text{CCW} - \tau_\text{CW}$. Analyizing the remaining torques:
\begin{itemize}
  ...
  \item $\text{E}$ is $2 \text{ m}$ from the pivot, and acting at a $-135^\circ$ angle from the horizontal. Our torque can be calculated from $\tau_\text{E} = (2)\text{E}\sin-135^\circ$. Note that usign $-45^\circ$ provides the same answer. This torque is also clockwise.
\end{itemize}
