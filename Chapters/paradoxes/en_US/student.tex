\chapter{Laws, Paradoxes, Thought Experiments, and the Illusions}
% placing this in book 11 as of april 2026
% I want this closer to the beginning as it is something all students should know generally. removing the following prerequisites since they are minor terms in the chapter. Generally these were prereqs but were only used once or twice so it was removed.
% probability_independent
% series
% parametric functions for the mobius strip
...
Further, some statements which this chapter explores is laws of science and mathematics. Some of these laws are not intuitive,
...
The resolution to Zeno's paradoxes lies in the concept of \newterm{limits} and \newterm{infinite series}. In modern mathematics, we understand that an infinite number of steps can be completed in a finite amount of time if the steps get smaller and smaller. If we consider the remaining distance to be covered as $1 \, \mathrm{unit}$, the first step is $\frac{1}{2}$, the second step is $\frac{1}{4}$, the third step is $\frac{1}{8}$, and so on. The total distance covered after $n$ steps can be expressed as a \newterm{geometric series}:
\[
...
\subsection{Time Travel Paradoxes}\index{time travel paradoxes}\index{grandfather paradox}
...
$$\mathbf{r}(u,v) =
    ...
    v\sin \tfrac{u}{2}
    \bigr)$$
where $u$ ranges from $0$ to $2\pi$ and $v$ ranges from $-1$ to $1$. Do not worry, this is not something you need to know how to do at all! We will cover parametric in a future workbook.
...
\section{Computer Science Paradoxes and Laws}
%ADD: Intro to computer science paradoxes
\subsection{Moravec's Paradox}
\newterm{Hans Moravec}, a pioneer in robotics and Artificial Intelligence, wrote in 1988,
\begin{quote}
...
What he means by this statement, referred to as the \newterm{Moravec's Paradox} is that tasks that are \emph{easy} for humans are \emph{hard} for machines, and tasks that are \emph{hard} for humans are relatively \emph{easy} for machines.
...
\begin{wrapfigure}{l}{0.6\textwidth}
    \begin{tikzpicture}
        \begin{axis}[
                ...
                height=6cm,
                axis lines=left,
                xlabel={Model/Data/Compute Scale},
                ylabel={Performance},
                xmin=0, xmax=8,
                ymin=0, ymax=10,
                samples=100,
