...
Without having to learn to much linear algebra, we can express this as a simple function: If we have a pixel coordinate expressed as a simple vector $\mathbf{p} = \vecb{x\\y}$, we can define a transformation function $T$ that takes this pixel coordinate and maps it to a new coordinate $\mathbf{p'}$:
$$\mathbf{p'} = T\mathbf{p}$$
