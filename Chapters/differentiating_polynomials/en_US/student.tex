...
\item The derivative of a nonconstant monomial $at^b$ ($a$ and $b$ are constant numbers, $t$ is time) is $abt^{b-1}$. This is referred to as the \textbf{power rule}. \index{power rule}
\end{itemize}\index{polynomials!differentiation}

\begin{mdframed}[frametitle = {Power Rule}, style = important]
For \( f(x) = ax^n \), the derivative is \( f'(x) = anx^{n-1} \).
\end{mdframed}

Say, for example, that we tell you that the height (position) in meters of a quadcopter
