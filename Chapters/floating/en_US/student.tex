...
\begin{tabular}{lccc}
    \hline
    & Total bits & Significand bits ($p$) & Exponent bits \\
    \hline
...
In both cases the layout is the same. The sign occupies the first bit, the significand
fills the next $p$ bits, and the exponent takes the remaining bits. Double precision
...
\begin{enumerate}
    ...
    \item If those exponent bits can represent integers from $-31$ to $32$, what is the
    largest exponent $M$?
...
\[
    u = \frac{1}{2} \beta^{1-p},
\]
where $\beta$ is the base and $p$ is the precision. For IEEE 754 double precision,
$\beta = 2$ and $p = 52$, giving $u \approx 2.22 \times 10^{-16}$. That means any
real number is stored with a relative error no worse than about $2 \times 10^{-16}$ [correct punctuation]
roughly 16 significant decimal digits of accuracy.

If you have access to Python, you can check your machine epsilon directly:
...
computations. It can also define integer data types, \texttt{int8}, \texttt{int32}, and
\texttt{int64}. These are used in situations where you only need whole numbers and want to avoid floating-point arithmetic entirely. 
