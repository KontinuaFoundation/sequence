...
(These are worth memorizing. Here's a mnemonic for the most common ones: ``King Henry Doesn't Usually Drink Chocolate Milk'' Or Kilo ($\times 10^{3}$), Hecto ($\times 10^{2}$), Deca ($\times 10^{1}$), Unit (for example: gram) ($\times 10^{0}$), Deci ($\times 10^{-1}$), Centi ($1\times 0^{-2}$), Milli ($\times 10^{-3}$).)
\end{mdframed}
...
Degrees celsius and degrees fahrenheit are \textit{not} proportional to
each other. If your food is $n$ degrees celsius, it is $n \times
\frac{9}{5} + 32$ degrees fahrenheit. You can't use conversion factors
