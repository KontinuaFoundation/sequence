...
To begin talking about limits, let's imagine a a scenario: 
...
\]
Now, if we were to flip the coin 100 times instead, and, we got $46$ heads. Our sample proportion gets closer to 0.5, which is our \newterm{expected value} or \newterm{mean}:
\[
...
This long-run stabilization is one version of the \newterm{Law of Large Numbers}, an important idea in statistics. 
...
\section{Limits of Asymptotes}
A function has a \textbf{vertical asymptote}\index{asymptote!vertical} at $x = a$ when the function values grow without bound as $x$ approaches $a$. A function's \textbf{end behavior}\index{end behavior} describes how the function values behave as $x \to \infty$ or $x \to -\infty$; if the function approaches a fixed value $L$, we say it has a \textbf{horizontal asymptote}\index{asymptote!horizontal} at $y = L$.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \begin{tikzpicture}
            \begin{axis}[width = 1\textwidth, xmin = 0.5, xmax = 1.5, axis lines = center, xlabel = $x$, ylabel = $f(x)$]
                \addplot[blue, thick, domain = 0.5:0.999]{(x^2 - 3*x + 2)/(x^2 - 4*x + 3)};
                \addplot[blue, thick, domain = 1.001:1.5]{(x^2 - 3*x + 2)/(x^2 - 4*x + 3)};
                \addplot[mark = o, blue]coordinates{(1, 0.5)};
            \end{axis}
        \end{tikzpicture}
        \caption{Zoomed out near $x=1$}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \centering
        \begin{tikzpicture}
            \begin{axis}[width = 1\textwidth, xmin = 0.9, xmax = 1.1, axis lines = center, xlabel = $x$, ylabel = $f(x)$]
                \addplot[blue, thick, domain = 0.9:0.9999]{(x^2 - 3*x + 2)/(x^2 - 4*x + 3)};
                \addplot[blue, thick, domain = 1.0001:1.1]{(x^2 - 3*x + 2)/(x^2 - 4*x + 3)};
                \addplot[mark = o, blue]coordinates{(1, 0.5)};
            \end{axis}
        \end{tikzpicture}
        \caption{Zoomed in closer to $x=1$}
    \end{subfigure}
    \caption{Two graphs of $f(x) = \frac{x^2 - 3x + 2}{x^2 - 4x + 3}$ zoomed in about $x = 1$}
    \label{holezoom}
\end{figure}
