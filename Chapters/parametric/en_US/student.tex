...
The central word you should think about as you go through this chapter is \emph{parameter}\index{parameter}. A parameter is a variable that both $x$ and $y$ depend on. The most common example of a parametric function is the circle. To represent a circle on a graph, we can use the equations
\begin{equation*}
    x = \cos(\theta) \quad \text{and} \quad y = \sin(\theta).
\end{equation*}


% figure yoinked from circular chapter
\begin{wrapfigure}{l}{0.5\textwidth}
    \centering
    \begin{tikzpicture}[declare function={angle=25;},bullet/.style={inner
                    sep=1pt,fill,draw,circle,solid}, scale=0.8] % scaled this down as it took up an entire page
        % Axis
    ...
    \end{tikzpicture}
    \caption{A diagram of a circle formed through a parametric functions.}
    \label{fig:sin_cos}
\end{wrapfigure}
...
%FIXME formatting within page
\begin{wrapfigure}{r}{0.5\textwidth}
    \centering
    \begin{tikzpicture}[scale=0.3]
        \draw[thick,-stealth,black] (-10,0)--(10,0) node[right] {$x$}; % x axis
        \draw[thick,-stealth,black] (0,-10)--(0,10) node[left] {$y$}; % y axis
        \draw[red, thick, smooth, samples=200, domain=0:3*pi] 
            plot ({\x*cos(\x r)}, {\x*sin(\x r)});
    \end{tikzpicture}
    \caption{Archimedian spiral.}
    \label{fig:param_arch_spiral}
\end{wrapfigure}

Here is an example of an Archimedian Spiral, a spiral where the radius $r$ is proportional to the angle $\theta$. In parametric form, it is represented as
...
In the Python animation, a point moves along the curve as the parameter $t$
increases. This animation highlights an important idea: parametric equations
do not only describe the shape of a curve, they describe motion along that
curve.

At each moment in time, the point is located at $(x(t), y(t))$. As $t$ changes,
the point moves from one position to the next. Sometimes this movement covers a
small distance, and sometimes it covers a larger distance, even though the
point remains on the same curve. This means the point does not necessarily move
at a constant rate.

Speed measures how fast the point is moving along the curve at a given value of
$t$. Once the speed is known, we can determine how far the point travels over a
time interval. The length of the curve traced by the parametric equations is the
total distance traveled by the moving point.

\subsection*{Speed of a Parametric Curve}

When a point moves along a parametric curve, its velocity is determined by the
derivatives $x'(t)$ and $y'(t)$. The speed of the point is the magnitude of the
velocity vector:
\begin{equation*}
    \text{Speed} = \sqrt{(x'(t))^2 + (y'(t))^2}.
\end{equation*}

Speed describes how fast the point is moving at a particular moment, but it
does not tell us the total distance traveled along the curve.

\subsection*{Arc Length Formula}

To find the length of the curve traced by a parametric equation from
$t = a$ to $t = b$, we integrate the speed over that interval.

If a curve is defined parametrically by $x = x(t)$ and $y = y(t)$, where
$x'(t)$ and $y'(t)$ are continuous on $[a,b]$, then the length of the curve is
given by
\begin{equation*}
    L = \int_a^b \sqrt{(x'(t))^2 + (y'(t))^2}\, dt.
\end{equation*}

\subsection*{Example: Finding Speed}
...
\begin{equation*}
    \text{Speed} =
    \sqrt{(8t - t^2)^2 + \left(-t + \sqrt{t^{1.2} + 20}\right)^2}.
\end{equation*}

Evaluating at $t = 2$ gives a speed of approximately $12.3$ cm/s.

\subsection*{Example: Length of a Curve}
...
Thus, the length of the curve traced is $2\pi$, which matches the circumference
of the unit circle.

\subsection*{Note}

Speed describes how fast a point moves along a parametric curve at a given
moment. Arc length measures the total distance traveled along the curve over an
interval of time. For parametric curves, arc length is found by integrating the
speed.
