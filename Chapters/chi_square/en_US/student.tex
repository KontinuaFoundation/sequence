...
%Expected counts are still computed with $E_{ij} = \frac{(\text{row total})(\text{column %total})}{\text{grand total}}$, and $df = (r-1)(c-1)$, exactly as before. In practice, the only %real difference on the page is how the row or column totals came to be: in a homogeneity %setting, one set of totals (say, the sample size from each group) was fixed by the design of %the study before any data were collected, rather than arising naturally from a single sample.
