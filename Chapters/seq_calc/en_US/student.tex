...
\section{The Idea of Approaching}
\subsection{Law of Large Numbers Revisited}
Recall that, in the limits chapter, we talked about the \newterm{Law of Large Numbers}. The Law states that as the number of tests (in the case from the limits chapter, coin flips) increases, the number of successes (heads) approaches closer to the expected value or mean.

Let's bring back our flip sequence from that chapter
\[
    H, T, T, H, T, H, T, T, H, T
\]

We want to make a sequence from this. Let's have the \[a_n = \frac{\text{heads so far}}{\text{flips so far}}\]
...
\[
    a_4 = \frac{2}{4} = 0.5
\]
And after all 10 flips, 
\[1,\frac{1}{2},\frac{1}{3},\frac{2}{4},\frac{2}{5},\frac{3}{6},\frac{3}{7},\frac{3}{8},\frac{4}{9},\frac{4}{10}\]
or as decimals,
\[
1,\ 0.5,\ 0.333,\ 0.5,\ 0.4,\ 0.5,\ 0.429,\ 0.375,\ 0.444,\ 0.4
\]
We represent this with the equation
\[
\bar X_n \to \mu
\]
the average value of $X$ approaches the true mean, $\mu$.

The Law of Large Numbers says that, over the long run, this sequence tends to settle near \(0.5\). In calculus, this kind of behavior has a name. When the terms of a sequence approach a fixed value as \(n\) gets larger, we say the sequence \newterm{converges}. If the terms do not approach one fixed value, we say the sequence \newterm{diverges}. Let's look at this with graphs and actual equations.

The coin-flip sequence is random, so now we will look at sequences whose rules are known exactly.
...
about this sequence? It isn't as easy to identify as our coin example. As $n$ increases, $a_n$ gets closer and closer to $1$.
...
Squeeze Theorem and l'H\^{o}pital's rule.
...
function, we can't use l'H\^{o}pital's rule. We can examine this sequence
