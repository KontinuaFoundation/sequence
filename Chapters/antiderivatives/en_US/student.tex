...
\centering
    \begin{tikzpicture}
        ...
        xmin = -1, xmax = 2.5, xtick = {0.5, 1, 1.5, 2},
        ymin = -2, ymax = 2, grid = major]
        	\addplot[blue, thick, domain = 0:5, samples=200]
        	{4*x^3-12*x^2+10*x-2};
        	\addplot[mark=*, blue, only marks]
        	coordinates{(0,-2) (0.293, 0) (0.592, 0.544) (1, 0) 
        	(1.408, -0.544) (1.707, 0) (2, 2)};
		\end{axis}
    \end{tikzpicture}
...
\centering
    \begin{tikzpicture}
		...
		[axis lines = center, xlabel = $x$, ylabel = $f(x)$,
        xmin = -1, xmax = 2.5, xtick = {0.5, 1, 1.5, 2},
        ymin = 0, ymax = 1.5, grid = major]
        	\addplot[mark=*, red, only marks]coordinates{(0,1)};
        	\addplot[red, dashed, thick, domain = -0.1:0.1]{-2*x+1};
        	\addplot[red, dashed, thick, domain = 0.2:0.4]{0.75};
        	\addplot[red, dashed, thick, domain = 0.5:0.7]{0.5*x+0.55};
        	\addplot[red, dashed, thick, domain = 0.9:1.1]{1};
        	\addplot[red, dashed, thick, domain = 1.3:1.5]{-0.4*x+1.42};
        	\addplot[red, dashed, thick, domain = 1.6:1.8]{0.75};
        	\addplot[red, dashed, thick, domain = 1.9:2.1]{2*x-3};
		\end{axis}
    \end{tikzpicture}
    \caption{Beginning sketch of $F(x)$}
    \label{fig:intfsketch}
\end{figure}

We can then connect these slopes to have an approximate sketch of 
$F(x)$:
...
\centering
    \begin{tikzpicture}
        ...
        xmin = -1, xmax = 2.5, xtick = {0.5, 1, 1.5, 2},
        ymin = 0, ymax = 1.5, grid = major]
        	\addplot[blue, thick, domain = -0.1:2.1]{1+x*(x-2)*(x-1)^2};
        	\addplot[mark=*, red, only marks]coordinates{(0,1)};
        	\addplot[red, dashed, thick, domain = -0.1:0.1]{-2*x+1};
        	\addplot[red, dashed, thick, domain = 0.2:0.4]{0.75};
        	\addplot[red, dashed, thick, domain = 0.5:0.7]{0.5*x+0.55};
...
antiderivative of $f(x)$ with respect to $x$ is denoted as $\int f(x)\,dx$. 
The $dx$ at the end indicates that we are integrating with
respect to $x$.
...
\section{Specific Antiderivatives}\index{antiderivatives!calculating +C}
...
for an example where $k = 2$.)\begin{figure}[H]
\centering

\begin{subfigure}{0.48\textwidth}
\centering
\begin{tikzpicture}[xscale=0.7, yscale=0.7]
	\begin{axis}[axis lines = center, legend pos = north west,
		xmin = -5, xmax = 5, xlabel = $x$,
		ymin = -5, ymax = 5, ylabel = $y$]
		\addplot[red, thick, samples=50, domain= 0.1:5]{1/x};
		\addlegendentry{$f(x) = \frac{1}{x}$}
		\addplot[blue, thick, samples=100, domain = 0.00001:5]{ln(x)};
		\addlegendentry{$\int \frac{1}{x}\,dx = \ln{|x|}$}
		\addplot[red, thick, samples=50, domain=-5:-0.001]{1/x};
		\addplot[blue, thick, samples=50, domain = -5:-0.001]{ln(-1*x)};
	\end{axis}
\end{tikzpicture}
\caption{$\frac{1}{x}$ and its antiderivative $\ln|x|$}
\label{fig:naturallog_sub}
\end{subfigure}\hfill% <-- no blank line here
\begin{subfigure}{0.48\textwidth}
\centering
\begin{tikzpicture}[xscale=0.7, yscale=0.7]
	\begin{axis}
		[xmin = -2, xmax = 2, xlabel = $x$, ylabel = $y$, ymax = 25,
		axis lines = center, legend pos = north west]
		\addplot[blue, thick, samples = 100, domain = -2:2]{e^(2*x)};
		\addlegendentry{$e^{2x}$}
		\addplot[red, thick, samples = 100, domain = -2:2]{0.5*e^(2*x)};
		\addlegendentry{$\frac{1}{2} e^{2x}$}
	\end{axis}	
\end{tikzpicture}
\caption{$e^{2x}$ and its antiderivative $\frac{1}{2}e^{2x}$}
\label{fig:antiexp_sub}
\end{subfigure}

\caption{Two examples of functions and their antiderivatives.}
\label{fig:anti_combined}
\end{figure}
...
\index{antiderivatives!higher order}
