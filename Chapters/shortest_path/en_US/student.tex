...
\section{Introduction — BFS revisited}

We have seen BFS as a way to search graphs (or trees). It is a pretty simple process: 
\begin{itemize}
    \item select a start node
    \item move one distance from the root, and visit all nodes ($v$) on that level
    \item when visiting, queue all child nodes ($v'$) of that node.
    \item keep going until either all nodes are marked \emph{visited} or a provided \emph{target node} is reached.
\end{itemize}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    % Edges
    \draw (A) -- node[lbl, left] {$\mathdollar1$} (B);
    \draw (B) -- node[lbl, below] {$\mathdollar2$} (C);
    \draw (A) -- node[lbl, right] {$\mathdollar6$} (C);

  \end{tikzpicture}
  \caption{A graph of 3 nodes that would fail BFS.}
  \label{fig:weighted_abc}
\end{figure}
...
To handle weighted graphs, we use a new algorithm that expands off of BFS, called \newterm{Dijkstra's Algorithm}. Dijkstra's Algorithm avoids the limitation of weights that BFS had by enqueueing the nodes in order of current shortest known distance.

\section{Dijkstra's Algorithm}

Edsger W. Dijkstra (pronounced \textit{DYKE-strah}) was a great Dutch computer scientist. He came up
