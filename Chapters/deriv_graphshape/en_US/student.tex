...
\index{increasing}\index{decreasing}
\textbf{Example 1}: Find where the function $f(x) = 3x^4-4x^3-12x^2+5$ is increasing. 

\textbf{Solution}: We want to find the intervals where $f'(x)>0$. First, we take the 
...
function, or didn't have the resources to graph it? 

We can use a table to help us analyze the value of $f'(x)$ (and therefore the behavior of $f(x)$). For 
...
\caption{Three functions with highlighted points.}
\end{figure}
...
\begin{wrapfigure}{r}{5cm}  % r = right side, width = 5cm
    \centering
    \begin{tikzpicture}
        \begin{axis}[width=4cm, xmin=0, xmax=6.28, axis lines=center]
...
the function's behavior at $\pm \infty$, if you are not restricted to a specific 
domain). 
...
straightforward way to identify global (absolute) extrema. 
...
\begin{enumerate}
	...
	\item $f(t) = \dfrac{\sqrt{t}}{1+t^2}$, $[0,2]$.
	...
	\[
	f(0) = 0, \qquad
	f\!\left(\frac{1}{\sqrt{3}}\right) = \frac{3^{3/4}}{4}, \qquad
	f(2) = \frac{\sqrt{2}}{5}.
	\]
	Since $\dfrac{3^{3/4}}{4}$ is the largest of these and $0$ is the smallest, the global maximum is
	\[
	...
	\item $f(t) = 2\cos t + \sin(2t)$, $\left[0,\frac{\pi}{2}\right]$.
	...
	\]
	On $\left[0,\frac{\pi}{2}\right]$, $\sin t = \frac{1}{2}$ gives $t = \frac{\pi}{6}$ (and $\sin t = -1$ is not in this interval).
	...
	= 2\cdot\frac{\sqrt{3}}{2} + \frac{\sqrt{3}}{2}
	= \frac{3\sqrt{3}}{2},
	\]
	\[
	f\!\left(\frac{\pi}{2}\right)
	= 2\cos\!\left(\frac{\pi}{2}\right) + \sin(\pi)
	= 0 + 0 = 0.
	\]
	...
	= \ln\!\left(\left(-\frac{1}{2}\right)^2 - \frac{1}{2} + 1\right)
	= \ln\!\left(\frac{1}{4} - \frac{1}{2} + 1\right)
...
\index{concavity!concave up}
\index{concavity!concave down}
\begin{figure}
\centering
\begin{tikzpicture}
