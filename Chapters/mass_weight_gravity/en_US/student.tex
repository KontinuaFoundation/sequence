...
The steps in sequence are shown below:
\begin{align*}
  F & = G \frac{Mm}{r^2}                   \\
    & = G \frac{Mm}{R_E^2}                 \\
...
Equation~\ref{eq:gravity} is the equation we will use a standard for the force of gravity, $F_g$, for most objects on Earth. Objects undergoing projectile motion, objects at rest, and objects hung by tension all have a force of gravity acting on them, \footnote{You may occasionally use 10 for g to simplify expressions if the problem says to do so, but use 9.8 otherwise.}
...
\begin{Exercise}[title=May the Force be with You 1, label=grav1]
  ...
  We won't ask it yet, but think to yourself: why doesn't the Earth move as much to the hammer as the hammer moves to the Earth?
\end{Exercise}
...
When larger bodies are under consideration for Newton's Universal Law of Gravity (Equation~\ref{eq:univGrav}), the gravitational force on both objects increases linearly. Bigger masses lead to greater forces between them. This is most noticeable with planets and moons; moons and planets keep each other in-orbit through the gravitational force between them.

However, despite the force increasing proportionally with mass, the resulting acceleration of each body depends inversely on its mass, so more massive bodies experience much smaller accelerations for the same interaction force. This is why the falling hammer from Exercise~\ref{grav1} doesn't "pull the Earth" towards it in any sense; since the acceleration is $a_\text{hammer} = \frac{F}{m_\text{hammer}}$, and the mass of the hammer is approximately $1 \, \mathrm{kg}$, the hammer accelerates a large amount. Even the force on the earth is the same, the acceleration $a_\text{Earth} = \frac{F}{m_\text{Earth}}$ is significantly less because the mass of the Earth is so great. The hammer and Earth both accelerate toward their common center of mass, the \newterm{center of mass} for the combination of the hammer and Earth system. That center lies almost exactly at Earth's center because Earth's mass dominates. See more about this in Section~\ref{sec:CoM}.

See the following below figure below:
...
\centering
  ...
  \foreach \a in {0,22.5,...,337.5} {
      \draw[->] ({3*cos(\a)},{3*sin(\a)}) -- ({2.2*cos(\a)},{2.2*sin(\a)});
    }
  \foreach \a in {0,22.5,...,337.5} {
      \draw[->] ({4*cos(\a)},{4*sin(\a)}) -- ({3.2*cos(\a)},{3.2*sin(\a)});
    }
  \end{tikzpicture}
  \caption{A figure of the direction of force vectors for a gravitational field.}
  \label{fig:grav_field}
\end{figure}

  All vectors in a gravitational field point to the center of mass of that field. The larger the body, as we said, the greater the force, and thus the greater magnitude of vectors. Thus, a gravitational field around a mass $M$ is a \newterm{vector field}, a term you will hear in vector calculus sections, or an assignment of a vector to every point in space. Every point has a force assignment based on the \textbf{radius} between the two objects.

  The gravitational field at a point in space is defined as the force per unit mass experienced by a small test mass placed at that point:
  $$\vec g = \frac{\vec F}{m}$$

  You'll notice $\vec g$ is the same as the acceleration! This is true, the gravitational field at a point is the acceleration any small test mass would have there. Therefore, points farther away from the body creating a gravitational field are smaller in magnitude. For symmetric and spherical bodies, this is true for all points revolved around the entire spherical body.
\section{Center of Mass}\label{sec:CoM}
...
In higher dimensions, it is easier to use integration to calculate the coordinates of the center of mass of some body, especially when it is not uniform (equally distributed). Although we haven't introduced integration formally, it is important to begin thinking about these topics:
