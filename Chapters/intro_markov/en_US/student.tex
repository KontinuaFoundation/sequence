...
\[
    \text{Probability Distribution after $k$ steps} = P^{k}
\]
...
For many Markov chains, these probabilities eventually stabilize (recall \textbf{Law of Large Numbers}, $P^k$ as $k \rightarrow \infty$) as we request more and more transitions or future states (recall Law of Large Numbers). That is, after a large number of transitions, the proportion of the probability of each state approaches a fixed value. This long-run behavior is known as the \newterm{steady-state distribution} or \newterm{equilibrium distribution} of the Markov chain.
    ...
    \begin{enumerate}
        \item We want to solve for $P\mathbf{x}=\mathbf{x}$. Let $\begin{array}{c} \mathbf{x}=
                      \begin{bmatrix}
                          x \\
                          y
                      \end{bmatrix}\end{array}$. Then we have
              $$\begin{bmatrix}
                      p   & 1-q \\
                      1-p & q
                  \end{bmatrix}
...
\subsection{Hidden Markov Models and NLP}
\newterm{Hidden Markov Models} (HMMs) are an expanded version of a Markov Process. In addition to having probabilities about transition from state to state, it also has influencing \newterm{observations}, called \newterm{emissions}. An emission is a \emph{visible} output generated by those the states in play, often which are hidden or not visible.

Assume you have a patient in a hospital with two possible states: $\mathcal{S} = \{\text{Sick}, \text{Healthy}\}$. You cannot know the exact state at a current time with certainty. However, you have three observable factors: $\mathcal{O} = \text{no fever}, \text{fever}, \text{cough}$. You may also see the emission states symbolized with $\mathcal{E}$.

For each state change observation (for this example, lets say once a day), you need both the transition matrix and the \newterm{emission matrix}. We will define the transition matrix $T$ and emission matrix $E$ as follows:
% sick | healthy
\[S = \begin{bmatrix}
        0.8 & 0.2 \\ % sick
        0.3 & 0.7 % healthy
    \end{bmatrix}, \qquad
    % no fever | fever | cough
    E=\begin{bmatrix}
        0.1 & 0.5 & 0.4 \\
        0.7 & 0.1 & 0.2
    \end{bmatrix}\]
...
% omitting since I believe it is not relevant to an intro chapter on such.
While we won't discuss them in this book, there are algorithms such as the Forward and Viterbi algorithms that can track and guess that most likely future state given the states available, or most likely emissions given a set of states. These use \emph{dynamic programming}, a method that programming similar to excel processing.

\subsection{Natural Language Processing}
Another strong use of HMM is the basics of Natural Language Processing, and ultimately, predictive AI text, such as reccommended words above your phone's keyboard.

Say we have a set of states represent parts of speech
$$\mathcal{S}=\{\text{Noun},\text{Verb},\text{Adjective}, \dots\}$$
...
$$\mathcal{O}=\{\text{dog},\text{runs},\text{fast},\dots \}$$

Suppose we are given the sentence \texttt{dog runs fast}. The observations are known, but the hidden states are not. One possible hidden-state sequence is

$$\texttt{Noun}\rightarrow \texttt{Verb}\rightarrow \texttt{Adverb}$$
...
$$P(\texttt{dog}∣\text{Noun})=0.4,
P(\texttt{runs}∣\text{Verb})=0.5$$

\begin{Exercise}[title={NLP Exercise}, label={ex:nlp}]

Suppose an HMM uses the following part-of-speech tags:

\[
\mathcal{S} = \{\text{Noun (N)}, \text{Verb (V)}, \text{Adjective (A)}\}.
\]
...
\[
\boxed{P(N \rightarrow V \rightarrow A)=0.10.}
\]

Thus, according to the model, the probability of observing the tag sequence

\[
\text{Noun} \rightarrow \text{Verb} \rightarrow \text{Adjective}
\]
