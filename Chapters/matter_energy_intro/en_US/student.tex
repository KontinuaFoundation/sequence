...
of water than there are drops of water in all the oceans. It is not easy to visualize the scale, but computers allow us to make simulations that can give us an idea. See \href{https://htwins.net/scale2/}{this cool website by Cary and Michael Huang} shows different size by scales! It may help to start at a length of $10^{-15.5}$ to see protons and neutrons for scale.
...
Notice how these atoms are mostly empty space in between the subatomic particles. The force, or attractive pull, keeping them together is called \textit{electromagnetic force}\index{electromagnetic force}. What keeps these charged atoms in their orbits is the \textbf{electromagnetic field}\index{electromagnetic field}. 

The charges of every particle create an \emph{electric field} around it. The orbitting and movement of an subatomic particles causes its \emph{magnetic field}. \textit{This is why electric current (moving electrons in a wire) produces a magnetic force. The faster the particles move, the stronger the field.} % this last sentence should be a footnote but it is interfering w/ fig2
...
\end{wrapfigure} %This figure has issues if it's near the bottom of the page. See Issue #60
...
Look around you: things are moving, warming up, lighting up, and changing. No physical property changes without \newterm{work} being done. What is work? No, we are not talking about having a job. Work is defined as a change in Energy. Every object has some type of energy. When you climb a flight of stairs, you feel tired. That tiredness is evidence that energy has left your body and gone somewhere else—into lifting your body upward. When the Sun warms your skin, energy is traveling through space and into you. When a moving car comes to a stop, its motion doesn't simply disappear; energy is transferred to the brakes and the surrounding air, mostly as heat.

Energy Transformations are all around us:
\begin{enumerate}
\item Feeling less tired after consuming calories from food
\item The light and the feelin of heat from the Sun
...
There are two main types of energy: energy that \emph{stored} and energy that is motion. Most times, energy that is stored is transformed into energy in motion. This stored energy is called \newterm{Potential Energy} and the energy in motion is referred to as \newterm{Kinetic Energy}. 
...
transfer energy as you lift the object against gravity. When you've lifted it as high as you can, most of the energy your body lost (we call this ``burning Calories'' colloquially) is stored as \textit{Potential Energy}, due to the object's height. Potential energy is the stored energy an object has due to its position or configuration, rather than its motion. \textit{Kinetic Energy} is the term we use to describe the energy an object possesses due to its motion; you'll learn more about different forms of energy in \ref{chap:conservationMassEnergy}, and in Sequence 2.
...
The Work-Energy theorem explains the relationship between work and energy, and we will introduce the theorem and use it to explain energy transfer in a subsequent chapter. 
