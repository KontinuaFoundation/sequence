...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=2]
      \def\L{4}           % rod length
      \def\maxAngle{35} 
      \def\bobRad{0.15}  % mass-ed bob radius
      
      \coordinate (pivot) at (0,4);
      \coordinate (eqPos) at (0,0);

      \draw[very thick, black!60] (pivot) -- (eqPos) node[midway] {Massless Rod};


      \begin{scope}[rotate around={35:(pivot)}]
        \filldraw[black!80] (0,0) circle (\bobRad); 
        \draw[very thick, black!60] (pivot) -- (0,0);
      \end{scope}


      \filldraw[black!80] (pivot) circle (0.1);
      \filldraw[black!80] (eqPos) circle (\bobRad);
