\chapter{Reflection and Photons}
% editing this to expand on the concepts of Photons
% Arjan - April 2026
...
photons. These photons travel at the \newterm{vacuum speed of light}, more often called the \textbf{\newterm{vacuum speed of light}}\index{speed of light}. This speed, in empty space or vacuums, is given by $2.9979 \times 10^8\,\textrm{m/s}$. Photons also
\begin{itemize}
  \item behave like particles and a wave.
  \item have zero mass 
  \item has zero charge
  \item can be created and destroyed when radiation is present
  \item release electrons upon collisions.
\end{itemize}
...
electromagnetic wave, and $h$ Planck's constant: $6.63 \times 10^{-34} m^2 kg/s$. Note that the angular frequency $f = \frac{c}{\lambda}$, and Planck's constant represents the quantum of action, establishing that energy is not continuous but exists in discrete packets called quantas. We will talk in a future chapter about how charge and energy is quantized, or packeted in integerized values which we call discrete.
...
For units sake, this is often abbreviated as $E = \frac{1240}{\lambda}$ where $\lambda$ is in nanometers and $E$ is in electronvolts ($\textrm{eV}$). 
\end{mdframed}

\subsection{The Photoelectric Effect}

In classical physics, scientists like Planck and Einstein thought that if you shined a light on a metal long enough or brightened it enough, it would eventually knock an electron loose, like a wave slowly pushing a boat to shore. Experiments showed that if the light was the wrong color, meaning too low of a frequency, nothing happened, no matter how bright or powerful the light was. 

This showed that light is quantized, and in order to travel up or down the energy levels of an atom, it must absorb or emit a photon with the exact energy difference between those levels. This phenomenon is known as the \newterm{photoelectric effect}.\index{photoelectric effect}

The effect is governed by the conservation of energy. The energy of an incoming photon ($E_{photon}$) must be greater than the bond holding the electron to the metal. This is described by $$E_{photon} = \Phi + K_{max}$$

where
\begin{itemize}
  \item $E_{\text{photon}} = hf$: The energy of the incoming photon, where $h$ is Planck's constant and $f$ is the frequency. 
  \item $\Phi$ (\newterm{Work Function}): The minimum energy required to eject an electron from a specific metal. Each material has a different ``cost'' to let an electron go.
  \item $f_0$ (\newterm{Threshold Frequency}): The specific frequency where the photon has exactly enough energy to match the work function ($\Phi = hf_0$). If $f < f_0$, no electrons are ejected. If $f = f_0$, electrons are released but have zero kinetic energy ($K_{\max} = 0$).
  \item $K_{\max}$: The ``leftover'' kinetic energy of the electron once released from the metal.
\end{itemize}
This is how solar panels and digital camera sensors turn light into electricity. Solar panels work like a one-way road for light energy. First, sunlight hits the panel in the form of photons, which act like tiny kicks that knock electrons loose from their atoms. Because the panel has a built-in internal electric field, these loose electrons are forced to move in only one direction (back into the panel), creating a steady flow of Direct Current (DC) electricity that can power your home. We are going to look at camera sensors in depth in a future workbook.
\subsection{Photons and Momentum}
In classical mechanics, we define momentum as the product of mass and velocity ($p = mv$). 

This leads to a common question: if a photon has zero rest mass, how can it possess momentum? Energy and momentum are linked, even for massless particles. For a photon, the momentum ($p$) is inversely proportional to its wavelength ($\lambda$):
\begin{equation}
  p = \frac{h}{\lambda} \quad \text{or} \quad p = \frac{E}{c}
  \label{eq:photon_momentum}
\end{equation}
\textbf{Application:}
For example, solar sails are spacecraft propulsion systems in space. These large, mirror-like sails in space can intercept millions of photons. As these photons reflect off the surface, they undergo a change in momentum ($\Delta p$), which, according to Newton's Second Law ($F = \frac{\Delta p}{\Delta t}$), exerts a constant force on the spacecraft, allowing for fuel-free propulsion. 
\subsection*{Photonic Interactions}
...
\section{Reflection}
%FIXME talk about how mirrors work, light why they are reflective?
...
The focus is at $(0, \frac{1}{a})$ and the directorix is the line $y = -\frac{1}{a}$.
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{Two parabolic dishes send waves between eachother.}
